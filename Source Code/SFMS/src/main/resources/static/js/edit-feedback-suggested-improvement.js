...
function getSuggestions() {
    ...
    let allQuestions = $("div[class='card-outer']");

    console.log(allQuestions.length);

    for (var i = 0; i < allQuestions.length; ++i) {
        //Lấy id của question để biết nó là question cũ hay mới tạo
        let questionID = allQuestions[i].querySelector("div[class='card-header-outer']").getAttribute("id");
        let suggestion = allQuestions[i].querySelector("input[class='form-control suggestion-input']").value;

        feedback.questions.push(new Question(questionID,suggestion));
    }
...
function saveSuggestion(feedback) {
    $.ajax({
        ...
        url: '/sfms/api/feedbacks/modify-suggestion',
        type: 'POST',
